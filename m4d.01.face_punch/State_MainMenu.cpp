...
State_MainMenu::State_MainMenu(StateManager* stateMgr)
	: BaseState(stateMgr), m_buttonPadding(0)
{
}
...
void State_MainMenu::OnCreate()
{
	...
	m_buttonSize = sf::Vector2f(300.f, 32.f);
...
void State_MainMenu::MouseClick(EventDetails* l_details)
{
	...
	for (int i = 0; i < 3; ++i)
	{
		auto rectPos = m_rects[i].getPosition();
		if (mousePos.x >= rectPos.x - halfX &&
			mousePos.x <= rectPos.x + halfX &&
			mousePos.y >= rectPos.y - halfY &&
			mousePos.y <= rectPos.y + halfY)
		{
			if (i == 0)
			{
			...
			{
				stateMgr_->GetContext()->m_wind->Close();
			}
		}
	}
}
